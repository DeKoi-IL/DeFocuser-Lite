...
## App
Each app instance owns one focuser (one COM port) and funnels commands from multiple ASCOM clients to that focus motor. Run one instance per focuser — e.g. a main scope and a piggybacked guide/lens defocuser.

The hub owns all configuration, including which focuser ASCOM clients talk to: the driver has no setup dialog of its own, and pressing Setup in NINA/SGP just brings the hub forward. With a single focuser there is nothing to configure — the first hub to connect claims the ASCOM slot. Running several, click **Use this hub** in the Connection panel of the one you want ASCOM to drive.
